...
def main(): 
    ...
    cursor = conn.cursor()
    cursor.execute('DROP TABLE "PrescriptionItems";')
    ...
    cursor.execute('CREATE TABLE "PrescriptionItems" (Medication VARCHAR(100), Dosage VARCHAR(50), PillCount INT, RefillDate DATE, ExpiryDate DATE, PillsUsedPerDay INT, DescriptionOfMedication VARCHAR(100), UserId VARCHAR(50), CONSTRAINT fk_constraint FOREIGN KEY (UserId) REFERENCES "Users"(UserId) ON DELETE CASCADE);')
    conn.commit()
    cursor.close()
    conn.close()
